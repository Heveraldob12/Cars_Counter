...
def Filter(img,filter):
    ...
    if filter=='combine':
        closing =cv2.morphologyEx(img, cv2.MORPH_CLOSE, Kernel('closing'), iterations=2) #recbendo os frames originhais
        opening = cv2.morphologyEx(closing, cv2.MORPH_OPEN, Kernel('opening'), iterations=2) # recebendo o resultado do closing
        dilation = cv2.dilate(opening, Kernel('dilation'), iterations=2) # recebendo o resultado do opening
        return dilation 
...
#------
w_min = 36 #Largura mínima
h_min = 40 # altura mínima
offset = 2 #Erro permitido
linha_ROI = 620 # Posição da linha 
carros = 0
